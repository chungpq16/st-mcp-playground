...
# Hardcoded OpenAI GPT-4o-mini setup - simplified imports
# DEFAULT_SYSTEM_PROMPT is defined locally since we don't need complex provider logic
DEFAULT_SYSTEM_PROMPT = """You are a helpful AI assistant that can help users with a variety of tasks. 
You have access to various tools and can help answer questions, analyze data, and perform calculations."""
...
def render_chat_history():
    ...
    if not st.session_state.chat_history:
        ...
        with st.container():
            st.markdown("""
            <div style="text-align: center; padding: 2rem; background-color: #2e3440; border-radius: 10px; margin: 1rem 0; color: #eceff4; border: 1px solid #4c566a;">
...
def handle_chat_input():
    ...
    # Check for generated prompt from sidebar
    generated_prompt = st.session_state.get('generated_prompt', '')
    
    # If there's a generated prompt, show it in a text area for editing
    if generated_prompt:
        st.write("**Generated Prompt from Sidebar:**")
        edited_prompt = st.text_area(
            "Edit the generated prompt if needed:",
            value=generated_prompt,
            height=150,
            key="edit_generated_prompt"
        )
        
        col1, col2 = st.columns([1, 4])
        with col1:
            if st.button("📝 Use This Prompt"):
                # Clear the generated prompt and submit the edited version
                st.session_state.generated_prompt = ""
                if edited_prompt.strip():
                    # Process as if it came from chat input
                    process_chat_submission(edited_prompt.strip())
                st.rerun()
        with col2:
            if st.button("❌ Cancel"):
                st.session_state.generated_prompt = ""
    ...
    if submission:
        process_chat_submission(submission)


def process_chat_submission(submission):
    """Process chat submission from chat input or generated prompt."""
...
def get_provider_capabilities(provider: str) -> Dict:
    ...
    # Hardcoded OpenAI GPT-4o-mini capabilities
    if provider != "openai":
        return {"Error": "Only OpenAI is supported"}
    
    return {
        "System Prompt Support": "✅",
        "Temperature Range": "0.0 - 2.0",
        "Max Tokens Range": "1 - 16,384",
        "Streaming Support": "✅",
        "Requires API Key": "✅"
    }
...
def render_system_prompt_section():
    ...
    if current_provider == "openai":  # OpenAI supports system prompts
...
def render_temperature_config(provider: str):
    ...
    temp_min, temp_max = 0.0, 2.0  # Hardcoded OpenAI temperature range
    default_temp = 0.7  # Hardcoded OpenAI default temperature
...
def render_max_tokens_config(provider: str):
    ...
    token_min, token_max = 1, 16384  # Hardcoded OpenAI token range
    default_tokens = 4096  # Hardcoded OpenAI default max tokens
...
def render_timeout_config(provider: str):
    ...
    default_timeout = 60  # Hardcoded OpenAI default timeout
...
def render_parameter_validation():
    ...
    if current_provider:
        ...
        is_valid = True  # Always valid for simplified setup
        error_msg = ""
...
def render_prompts_tab():
    """Render the MCP prompts management tab."""
    from .mcp_client import MCPConnectionManager
    
    st.header("🎯 MCP Prompts")
    st.write("Browse and test prompts from your MCP servers.")
    
    # Get MCP connection manager
    manager = MCPConnectionManager.get_instance()
    
    if not manager.is_connected:
        st.warning("⚠️ MCP client is not connected. Please configure and start MCP servers in the configuration tab.")
        return
    
    if not manager.server_config:
        st.warning("⚠️ No MCP server configuration found.")
        return
    
    # Server selection
    server_names = list(manager.server_config.keys())
    if not server_names:
        st.warning("⚠️ No MCP servers configured.")
        return
    
    selected_server = st.selectbox(
        "📡 Select MCP Server:",
        server_names,
        help="Choose which MCP server to get prompts from"
    )
    ...
    col1, col2 = st.columns([1, 4])
    with col1:
        refresh_prompts = st.button("🔄 Refresh Prompts", help="Refresh prompts from server")
    
    # Get prompts from selected server
    try:
        with st.spinner(f"Loading prompts from {selected_server}..."):
            prompts = run_async(manager.get_prompts(selected_server, force_refresh=refresh_prompts))
        
        if not prompts:
            st.info(f"ℹ️ No prompts found on server '{selected_server}'.")
            return
        
        st.success(f"✅ Found {len(prompts)} prompts on server '{selected_server}'")
            ...
            with st.expander(f"🎯 {prompt['name']}", expanded=False):
                ...
                if prompt.get('arguments'):
                    ...
                    for arg in prompt['arguments']:
                        required_badge = "🔴 Required" if arg.get('required') else "🟡 Optional"
                        st.write(f"- **{arg['name']}** ({required_badge}): {arg.get('description', 'No description')}")
                ...
                if prompt.get('arguments'):
                    for arg in prompt['arguments']:
                        arg_key = f"prompt_arg_{selected_server}_{prompt['name']}_{arg['name']}"
                        if arg.get('required'):
                ...
                # Test button
                test_key = f"test_prompt_{selected_server}_{prompt['name']}"
                if st.button(f"🧪 Test Prompt", key=test_key):
                    ...
                    if missing_required:
                        ...
                        try:
                            with st.spinner(f"Getting prompt '{prompt['name']}'..."):
                                messages = run_async(manager.get_prompt(
                                    selected_server, 
                                    prompt['name'], 
                                    filtered_args if filtered_args else None
                                ))
                            
                            st.success(f"✅ Prompt '{prompt['name']}' retrieved successfully!")
                            ...
                            st.error(f"❌ Failed to get prompt: {format_error_message(e)}")
    
    except Exception as e:
        st.error(f"❌ Failed to load prompts: {format_error_message(e)}")
        
        # Show debug info in expander
        with st.expander("🔍 Debug Information"):
            st.write(f"**Server Config:** {manager.server_config}")
            st.write(f"**Is Connected:** {manager.is_connected}")
